...
const getProducts = async ({start, sort, category }: {
        start: number, 
        sort: ProductSort,
        category: string,
...
export default (args: { start: number, sort: ProductSort, category: string }) => new Promise<{ products: Product[], total: number, skip: number, limit: number }>((resolve) => {
    setTimeout(() => {
